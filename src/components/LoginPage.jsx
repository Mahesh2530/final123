...
export function LoginPage({ onLogin, onCreateAccount }) {
    ...
    try {
      ...
      if (user && user.password === password) {
        ...
        if (user.role === 'admin' && user.isBlocked) {
          alert("Your account has been blocked due to excessive poor reviews. Please contact support.")
          setLoading(false)
          return
        }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-400 via-black to-yellow-600 flex items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-2xl border-4 border-yellow-300 animate-scale-in">
        <CardHeader className="space-y-2 text-center bg-gradient-to-r from-yellow-400 via-black to-yellow-500">
          <div className="flex justify-center mb-4">
            <div className="bg-yellow-300 p-3 rounded-lg shadow-lg animate-bounce-gentle">
        ...
        <CardContent className="space-y-6 pt-8 bg-gradient-to-b from-black to-yellow-900">
          <div className="space-y-4">
            <Input
              ...
              disabled={loading}
              className="border-2 border-yellow-400 focus:border-yellow-300 font-semibold bg-black text-yellow-100 placeholder-yellow-600 animate-slide-in-left focus-scale"
            />
            <Input
              ...
              onKeyPress={handleKeyPress}
              disabled={loading}
              className="border-2 border-yellow-400 focus:border-yellow-300 font-semibold bg-black text-yellow-100 placeholder-yellow-600 animate-slide-in-right focus-scale"
            />
            ...
            <Button
              onClick={handleLogin}
              disabled={loading}
              className="w-full bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 text-black font-bold text-base h-12 shadow-lg hover-scale hover-shadow disabled:opacity-50 animate-fade-scale stagger-1"
              size="lg"
            >
